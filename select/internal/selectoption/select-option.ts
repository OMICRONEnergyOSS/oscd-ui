 ...
 * must have `oscd-list-item` and `oscd-menu-item` attributes on the host.
 */
export type SelectOption = SelectOptionSelf & MenuItem;

// Separate variable needed for closure.
const selectOptionBaseClass = mixinDelegatesAria(
 ...
 * @fires request-selection {Event} Requests the parent oscd-select to select this
 * element (and deselect others if single-selection) when `selected` changed to
 * `true`. --bubbles --composed
 * @fires request-deselection {Event} Requests the parent oscd-select to deselect
  ...
  implements SelectOption
{
   ...
   */
  @property({ type: Boolean, attribute: 'oscd-menu-item', reflect: true })
  isMenuItem = true;
  ...
  protected renderListItem(content: unknown) {
    return html`
      <li
        ...
        ?aria-label=${!!(this as ARIAMixinStrict).ariaLabel}
        aria-selected=${!!(this as ARIAMixinStrict).ariaSelected}
        aria-checked=${!!(this as ARIAMixinStrict).ariaChecked}
        aria-expanded=${!!(this as ARIAMixinStrict).ariaExpanded}
        aria-haspopup=${!!(this as ARIAMixinStrict).ariaHasPopup}
