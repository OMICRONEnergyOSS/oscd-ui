...
/**
 * Creates an event fired by a SelectOption to request selection from oscd-select.
 ...
 * oscd-select. Typically fired after `selected` changes from `true` to `false`.
 */
...
/**
 * A controller that provides most functionality and oscd-select compatibility for
...
export class SelectOptionController implements ReactiveController {
  ...
  hostUpdated() {
    ...
    if (this.lastSelected !== this.host.selected && !this.firstUpdate) {
      ...
      // handled by oscd-select because it needs to coordinate the
