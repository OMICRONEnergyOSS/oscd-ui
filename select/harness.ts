...
export class SelectHarness extends Harness<Select> {
  ...
  async clickOption(index: number) {
    const menu = this.element.renderRoot.querySelector('oscd-menu')!;
    if (!menu.open) {
      console.warn(
  ...
  get isOpen() {
    ...
    if (!menu) {
      throw new Error(
        'Internal oscd-menu is not found. oscd-select may not have finished rendering when isOpen has been checked',
      );
    }
    return menu.open;
  }
