...
describe('OscdFilledSelect side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdFilledSelect } = await import('./OscdFilledSelect.js');

    // Instantiate the component
    const oscdFilledSelect = await fixture(
      html`<oscd-filled-select></oscd-filled-select>`,
      { scopedElements: { 'oscd-filled-select': OscdFilledSelect } },
    );

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdFilledSelect.shadowRoot).not.to.be.null;
    expect(oscdFilledSelect.constructor.name).to.equal('OscdFilledSelect');
