...
describe('OscdFab side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdFab } = await import('./OscdFab.js');

    // Instantiate the component
    const oscdFab = await fixture(html`<oscd-fab></oscd-fab>`, {
      scopedElements: { 'oscd-fab': OscdFab },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdFab.shadowRoot).not.to.be.null;
    expect(oscdFab.constructor.name).to.equal('OscdFab');
