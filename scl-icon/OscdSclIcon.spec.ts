...
describe('OscdIcon side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdSclIcon } = await import('./OscdSclIcon.js');

    // Instantiate the component
    const oscdIcon = await fixture(html`<oscd-icon></oscd-icon>`, {
      scopedElements: { 'oscd-icon': OscdSclIcon },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdIcon.shadowRoot).not.to.be.null;
    expect(oscdIcon.constructor.name).to.equal('OscdSclIcon');
