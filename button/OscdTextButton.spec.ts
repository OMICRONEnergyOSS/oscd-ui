...
describe('OscdTextButton side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdTextButton } = await import('./OscdTextButton.js');

    // Instantiate the component
    const oscdTextButton = await fixture(
      html`<oscd-text-button></oscd-text-button>`,
      { scopedElements: { 'oscd-text-button': OscdTextButton } },
    );

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdTextButton.shadowRoot).not.to.be.null;
    expect(oscdTextButton.constructor.name).to.equal('OscdTextButton');
