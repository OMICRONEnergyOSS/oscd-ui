...
describe('OscdCheckbox side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdCheckbox } = await import('./OscdCheckbox.js');

    // Instantiate the component
    const oscdCheckbox = await fixture(html`<oscd-checkbox></oscd-checkbox>`, {
      scopedElements: { 'oscd-checkbox': OscdCheckbox },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdCheckbox.shadowRoot).not.to.be.null;
    expect(oscdCheckbox.constructor.name).to.equal('OscdCheckbox');
