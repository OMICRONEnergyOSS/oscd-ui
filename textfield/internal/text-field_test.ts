...
declare global {
  interface HTMLElementTagNameMap {
    'oscd-test-text-field': TestTextField;
  }
}

@customElement('oscd-test-text-field')
class TestTextField extends TextField {
  ...
  async getErrorTextValue() {
    await this.updateComplete;
    return this.renderRoot.querySelector('oscd-filled-field')?.errorText || '';
  }
}
...
describe('TextField', () => {
  ...
  async function setupTest(
    template = html`<oscd-test-text-field></oscd-test-text-field>`,
  ) {
    // Variant type does not matter for shared tests
    const element = env.render(template).querySelector('oscd-test-text-field');
    if (!element) {
      throw new Error('Could not query rendered <oscd-test-text-field>.');
    }
  ...
  describe('forms', () => {
    createFormTests({
      queryControl: root => root.querySelector('oscd-test-text-field'),
      valueTests: [
        {
          name: 'unnamed',
          render: () =>
            html`<oscd-test-text-field value="Value"></oscd-test-text-field>`,
        ...
        },
        {
          ...
          render: () =>
            html`<oscd-test-text-field name="input"></oscd-test-text-field>`,
          assertValue(formData) {
        ...
        },
        {
          name: 'with value',
          render: () =>
            html`<oscd-test-text-field
              name="input"
              value="Value"
            ></oscd-test-text-field>`,
          assertValue(formData) {
