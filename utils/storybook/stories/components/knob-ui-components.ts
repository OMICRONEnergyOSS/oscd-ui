...
export class KnobColorSelector extends LitElement {
  ...
  private renderTextInput() {
    return html`<oscd-filled-text-field
  ...
  private renderColorInput() {
    ...
    return html`
      <oscd-ripple for="color-picker" style=${rippleStyles}></oscd-ripple>
      <input
        type="color"
        id="color-picker"
        .value=${this.value}
 ...
 */
export function textInput(): KnobUi<string>;
...
export function button(): KnobUi<number> {
  return {
    render(knob, onChange) {
      ...
      return html`
        <oscd-outlined-button outlined @click=${onClick} style=${styles}>
          ${knob.name}
        </oscd-outlined-button>
      `;
    },
...
export function setProp(selector: string) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- No way to represent this type clearly.
  return (knob: Knob<any>, val: unknown, container: HTMLElement) => {
    ...
    for (const element of elements) {
      if (!defaults.has(element)) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Necessary for setting arbitrary props
