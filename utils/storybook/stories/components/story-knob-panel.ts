...
export class StoryKnobPanel extends LitElement {
  ...
  @property({ type: String, reflect: true }) type: 'modal' | 'inline' =
    'inline';
  ...
  override render() {
    ...
    if (this.showCloseIcon) {
      closeIcon = html`
        <oscd-icon-button aria-label="close" @click=${this.close}>
    ...
    if (this.draggable) {
      dragBar = html`
        ...
        >
          <oscd-icon>drag_handle</oscd-icon>
        </div>
      `;
    }
