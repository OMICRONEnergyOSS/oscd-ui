...
describe('OscdSlider side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdSlider } = await import('./OscdSlider.js');

    // Instantiate the component
    const oscdSlider = await fixture(html`<oscd-slider></oscd-slider>`, {
      scopedElements: { 'oscd-slider': OscdSlider },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdSlider.shadowRoot).not.to.be.null;
    expect(oscdSlider.constructor.name).to.equal('OscdSlider');
