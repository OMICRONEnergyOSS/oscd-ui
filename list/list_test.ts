...
describe('<oscd-list>', () => {
  ...
  describe('.styles', () => {
    createTokenTests(OscdList.styles);
  });
  ...
  describe('keyboard navigation', () => {
    it('non-nagivable key does nothing', async () => {
      const root = env.render(
        html` <oscd-list>
          <oscd-list-item type="button" tabindex="-1"></oscd-list-item>
          <oscd-list-item type="button" tabindex="0"></oscd-list-item>
          <oscd-list-item type="button" tabindex="-1"></oscd-list-item>
        </oscd-list>`,
      );

      const listEl = root.querySelector('oscd-list')!;
      const listHarness = new ListHarness(listEl);
      const [first, second, third] = Array.from(
        root.querySelectorAll('oscd-list-item'),
      );
    ...
    it('ArrowDown will do nothing if nothing is selectable', async () => {
      const root = env.render(
        html` <oscd-list>
          <oscd-list-item type="button" disabled></oscd-list-item>
          <oscd-list-item type="button" disabled></oscd-list-item>
        </oscd-list>`,
      );
      const listEl = root.querySelector('oscd-list')!;
    ...
    it('ArrowDown does not activate disabled items', async () => {
      const root = env.render(
        html` <oscd-list>
          <oscd-list-item type="button"></oscd-list-item>
          <oscd-list-item type="button" disabled></oscd-list-item>
          <oscd-list-item type="button"></oscd-list-item>
        </oscd-list>`,
      );
    ...
    it('ArrowDown will select itself if its the only activatable', async () => {
      ...
      const listEl = root.querySelector('oscd-list')!;
      const listHarness = new ListHarness(listEl);
      const first = root.querySelector('oscd-list-item')!;
  ...
  describe('activate items methods', () => {
    ...
    it('List will activate first activatable item if defined by user', async () => {
      const root = env.render(
        html` <oscd-list>
          <oscd-list-item type="button" tabindex="-1"></oscd-list-item>
          <oscd-list-item type="button" tabindex="2"></oscd-list-item>
          <oscd-list-item type="button" tabindex="1"></oscd-list-item>
        </oscd-list>`,
      );
      const [first, second, third] = Array.from(
    ...
    it('activateNextItem will return null if nothing is selectable', async () => {
      const root = env.render(html` <oscd-list> </oscd-list>`);
      const listEl = root.querySelector('oscd-list')!;
  ...
  describe('aria', () => {
    ...
    it('Does not override user given role attribute', async () => {
      const root = env.render(
        html`<oscd-list role="listbox" aria-label="Options"></oscd-list>`,
      );
      const listEl = root.querySelector('oscd-list')!;
      await env.waitForStability();
    ...
    it('Does not override user given role property', async () => {
      const root = env.render(html`<oscd-list .role=${'listbox'}></oscd-list>`);
      const listEl = root.querySelector('oscd-list')!;
      await env.waitForStability();
...
describe('<oscd-list-item>', () => {
  ...
  describe('rendering', () => {
    it('self-describes as a list-item', async () => {
      const root = env.render(html` <oscd-list-item> </oscd-list-item>`);

      const listItemEl = root.querySelector('oscd-list-item')!;

      await env.waitForStability();

      expect(listItemEl.hasAttribute('oscd-list-item')).toBeTrue();
    });
  });

  it('disabled overrides tabIndex', async () => {
    const root = env.render(
      html`<oscd-list-item type="button"></oscd-list-item>`,
    );

    const listItem = root.querySelector('oscd-list-item')!;
  ...
  it('ripple and focus ring rendered on interactive', async () => {
    ...
    let rippleEl = listItem.renderRoot.querySelector('oscd-ripple');
    let focusRingEl = listItem.renderRoot.querySelector('oscd-focus-ring');
    ...
    rippleEl = listItem.renderRoot.querySelector('oscd-ripple');
    focusRingEl = listItem.renderRoot.querySelector('oscd-focus-ring');
...
describe('<oscd-list-item> link', () => {
  ...
  it('setting href renders an anchor tag', async () => {
    const root = env.render(
      html`<oscd-list-item href="https://google.com"></oscd-list-item>`,
    );
  ...
  it('setting target without href renders nothing', async () => {
    const root = env.render(
      html`<oscd-list-item target="_blank"></oscd-list-item>`,
    );
