...
describe('OscdListItem side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdListItem } = await import('./OscdListItem.js');

    // Instantiate the component
    const oscdListItem = await fixture(
      html`<oscd-list-item></oscd-list-item>`,
      { scopedElements: { 'oscd-list-item': OscdListItem } },
    );

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdListItem.shadowRoot).not.to.be.null;
    expect(oscdListItem.constructor.name).to.equal('OscdListItem');
