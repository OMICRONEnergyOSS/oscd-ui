...
describe('OscdAssistChip side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdAssistChip } = await import('./OscdAssistChip.js');

    // Instantiate the component
    const oscdAssistChip = await fixture(
      html`<oscd-assist-chip></oscd-assist-chip>`,
      { scopedElements: { 'oscd-assist-chip': OscdAssistChip } },
    );

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdAssistChip.shadowRoot).not.to.be.null;
    expect(oscdAssistChip.constructor.name).to.equal('OscdAssistChip');
