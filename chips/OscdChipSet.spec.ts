...
describe('OscdChipSet side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdChipSet } = await import('./OscdChipSet.js');

    // Instantiate the component
    const oscdChipSet = await fixture(html`<oscd-chip-set></oscd-chip-set>`, {
      scopedElements: { 'oscd-chip-set': OscdChipSet },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdChipSet.shadowRoot).not.to.be.null;
    expect(oscdChipSet.constructor.name).to.equal('OscdChipSet');
