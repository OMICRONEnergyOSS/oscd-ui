 ...
 *
 */
export class Tabs extends ScopedElementsMixin(LitElement) {
   ...
   */
  @queryAssignedElements({ flatten: true, selector: '[oscd-tab]' })
  readonly tabs!: Tab[];
  ...
  set activeTabIndex(index: number) {
    ...
    if (!this.slotElement) {
      ...
      // ```ts
      // html`
      //   <oscd-tabs .activeTabIndex=${1}>
      //     <oscd-tab>First</oscd-tab>
      //     <oscd-tab>Second</oscd-tab>
      //   </oscd-tabs>
      // `;
      // ```
      //
  ...
  protected override render() {
    return html`
      ...
      <oscd-divider part="divider"></oscd-divider>
    `;
  }
  ...
  private activateTab(activeTab: Tab) {
    ...
    if (previousTab) {
      ...
      // were selected, such as when oscd-tabs auto-selects the first tab.
...
function isTab(element: unknown): element is Tab {
  return element instanceof HTMLElement && element.hasAttribute('oscd-tab');
}
