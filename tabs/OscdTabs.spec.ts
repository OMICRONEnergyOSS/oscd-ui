...
describe('OscdTabs side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdTabs } = await import('./OscdTabs.js');

    // Instantiate the component
    const oscdTabs = await fixture(html`<oscd-tabs></oscd-tabs>`, {
      scopedElements: { 'oscd-tabs': OscdTabs },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdTabs.shadowRoot).not.to.be.null;
    expect(oscdTabs.constructor.name).to.equal('OscdTabs');
