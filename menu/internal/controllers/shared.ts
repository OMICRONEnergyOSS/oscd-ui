...
/**
 * The interface needed for a Menu to work with other oscd-menu elements.
 */
...
/**
 * The interface needed for a Menu to work with other oscd-menu elements. Useful
 * for keeping your types safe when wrapping `oscd-menu`.
 */
 ...
 * `oscd-menu-item` are, Space, Enter or Escape.
 */
...
/**
 * The default menu closing reasons for the material oscd-menu package.
 */
...
/**
 * Creates a default close menu event used by oscd-menu.
 */
export const createDefaultCloseMenuEvent = createCloseMenuEvent<DefaultReasons>;

/**
 * The type of the default close menu event used by oscd-menu.
 */
...
export function isElementInSubtree(
  ...
  const focusEv = new Event('oscd-contains', { bubbles: true, composed: true });
  let composedPath: EventTarget[] = [];
  const listener = (ev: Event) => {
    composedPath = ev.composedPath();
  };

  container.addEventListener('oscd-contains', listener);
  target.dispatchEvent(focusEv);
  container.removeEventListener('oscd-contains', listener);
