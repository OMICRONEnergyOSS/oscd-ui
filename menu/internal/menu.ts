...
export abstract class Menu extends ScopedElementsMixin(LitElement) {
   ...
   * when placed inside of them. e.g. using a menu inside of an `oscd-dialog`.
   *
   ...
   * - You are reusing a single `oscd-menu` element that dynamically renders
   *   content.
   *
   ...
   * `tabindex` to `0` and change oscd-menu's display to something other than
  ...
  private readonly listController = new ListController<MenuItem>({
    isItem: (maybeItem: HTMLElement): maybeItem is MenuItem => {
      return maybeItem.hasAttribute('oscd-menu-item');
    },
    ...
    isNavigableKey: (key: string) => {
      ...
      // we want oscd-submenu to handle the submenu's left/right arrow exit
   ...
   * have both the `oscd-menu-item` and `oscd-list-item` attributes.
   */
  get items(): MenuItem[] {
  ...
  private renderElevation() {
    return html`<oscd-elevation part="elevation"></oscd-elevation>`;
  }
  ...
  private readonly handleFocusout = async (event: FocusEvent) => {
    ...
    if (event.relatedTarget) {
      ...
      // oscd-menu-item, and oscd-list or if the anchor was click focused, but check
  ...
  private async animateOpen() {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      // animation.
      child.classList.toggle('oscd-menu-hidden', true);
      animation.addEventListener('finish', () => {
        child.classList.toggle('oscd-menu-hidden', false);
      });
  ...
  private handleDeactivateTypeahead(event: DeactivateTypeaheadEvent) {
    ...
    // nested above it e.g. oscd-sub-menu
    event.stopPropagation();
