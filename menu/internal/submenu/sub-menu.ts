...
export class SubMenu extends LitElement {
   ...
   */
  @property({ type: Number, attribute: 'hover-open-delay' }) hoverOpenDelay =
    400;
  /**
   ...
   */
  @property({ type: Boolean, reflect: true, attribute: 'oscd-sub-menu' })
  isSubMenu = true;
  ...
  async show() {
    ...
    // submenu's `<oscd-menu slot="submenu" positioning="document">` would be
    // wrong even if we change `oscd-sub-menu` from `position: relative` to
    ...
    // user hovers a parent menu's item which is not an oscd-sub-menu item.
    ...
    // around the menu and then mouses over an oscd-sub-menu.
    this.dispatchEvent(createDeactivateItemsEvent());
