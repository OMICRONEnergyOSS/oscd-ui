...
describe('OscdSubMenu side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdSubMenu } = await import('./OscdSubMenu.js');

    // Instantiate the component
    const oscdSubMenu = await fixture(html`<oscd-sub-menu></oscd-sub-menu>`, {
      scopedElements: { 'oscd-sub-menu': OscdSubMenu },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdSubMenu.shadowRoot).not.to.be.null;
    expect(oscdSubMenu.constructor.name).to.equal('OscdSubMenu');
