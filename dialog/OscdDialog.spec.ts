...
describe('OscdDialog side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdDialog } = await import('./OscdDialog.js');

    // Instantiate the component
    const oscdDialog = await fixture(html`<oscd-dialog></oscd-dialog>`, {
      scopedElements: { 'oscd-dialog': OscdDialog },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdDialog.shadowRoot).not.to.be.null;
    expect(oscdDialog.constructor.name).to.equal('OscdDialog');
