...
describe('OscdBadge side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdBadge } = await import('./OscdBadge.js');

    // Instantiate the component
    const oscdBadge = await fixture(html`<oscd-badge></oscd-badge>`, {
      scopedElements: { 'oscd-badge': OscdBadge },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdBadge.shadowRoot).not.to.be.null;
    expect(oscdBadge.constructor.name).to.equal('OscdBadge');
