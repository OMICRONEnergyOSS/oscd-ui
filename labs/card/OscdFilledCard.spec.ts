...
describe('OscdFilledCard side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdFilledCard } = await import('./OscdFilledCard.js');

    // Instantiate the component
    const oscdFilledCard = await fixture(
      html`<oscd-filled-card></oscd-filled-card>`,
      { scopedElements: { 'oscd-filled-card': OscdFilledCard } },
    );

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdFilledCard.shadowRoot).not.to.be.null;
    expect(oscdFilledCard.constructor.name).to.equal('OscdFilledCard');
