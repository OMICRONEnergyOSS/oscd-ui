...
>(base: T): MixinReturn<T, OnReportValidity> {
    ...
    implements OnReportValidity
  {
    ...
    [onReportValidity](_invalidEvent: Event | null) {
      throw new Error(
        `Implement [onReportValidity] in ${this.constructor.name}`,
      );
    }
...
function getFormValidateHooks(form: HTMLFormElement) {
  if (!FORM_VALIDATE_HOOKS.has(form)) {
    ...
    for (const methodName of ['reportValidity', 'requestSubmit'] as const) {
      ...
      form[methodName] = function (this: HTMLFormElement, ...args: unknown[]) {
        hooks.dispatchEvent(new Event('before'));
        const result = Reflect.apply(superMethod, this, args);
