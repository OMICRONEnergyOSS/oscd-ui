...
describe('OscdItem side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdItem } = await import('./OscdItem.js');

    // Instantiate the component
    const oscdItem = await fixture(html`<oscd-item></oscd-item>`, {
      scopedElements: { 'oscd-item': OscdItem },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdItem.shadowRoot).not.to.be.null;
    expect(oscdItem.constructor.name).to.equal('OscdItem');
