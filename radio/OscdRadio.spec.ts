...
describe('OscdRadio side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdRadio } = await import('./OscdRadio.js');

    // Instantiate the component
    const oscdRadio = await fixture(html`<oscd-radio></oscd-radio>`, {
      scopedElements: { 'oscd-radio': OscdRadio },
    });

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdRadio.shadowRoot).not.to.be.null;
    expect(oscdRadio.constructor.name).to.equal('OscdRadio');
