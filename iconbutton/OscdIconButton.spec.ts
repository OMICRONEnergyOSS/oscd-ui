...
describe('OscdIconButton side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdIconButton } = await import('./OscdIconButton.js');

    // Instantiate the component
    const oscdIconButton = await fixture(
      html`<oscd-icon-button></oscd-icon-button>`,
      { scopedElements: { 'oscd-icon-button': OscdIconButton } },
    );

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdIconButton.shadowRoot).not.to.be.null;
    expect(oscdIconButton.constructor.name).to.equal('OscdIconButton');
