...
describe('OscdFilledField side-effect free', () => {
  ...
  it('should not call customElements.define as a side effect during import/instantiation', async () => {
    ...
    const { OscdFilledField } = await import('./OscdFilledField.js');

    // Instantiate the component
    const oscdFilledField = await fixture(
      html`<oscd-filled-field></oscd-filled-field>`,
      { scopedElements: { 'oscd-filled-field': OscdFilledField } },
    );

    // Verify it's upgraded (not just a dummy tag)
    expect(oscdFilledField.shadowRoot).not.to.be.null;
    expect(oscdFilledField.constructor.name).to.equal('OscdFilledField');
